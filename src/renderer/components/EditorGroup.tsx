      ...
      {openFiles.map((f, i) => {
        ...
        return (
          <div
            ...
            style={{ maxWidth: 180 }}
          >
            <span className="tab-name" style={{ maxWidth: 140 }}>{f.name}</span>
            {dirty && <span className="tab-modified" />}
          </div>
        );
      })}
      <button className="split-close-btn" onClick={() => onCloseSplit(side)} title="Close split" aria-label="Close split">
