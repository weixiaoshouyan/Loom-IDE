...
export default function SidebarExplorerView({ workspacePath, onOpenFile, onOpenFolder, onCloseFolder, selectedFile, gitStatusMap, locale }: {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('loom:notify', { detail: { message: `创建失败: ${e.message}`, type: 'error' } }));
    }
    setCreating(null);
    setNewName('');
  };
      ...
      <div className="sidebar-header">
        <span title={workspacePath}>
          {workspacePath
            ...
            : (locale === 'zh-CN' ? '资源管理器' : 'EXPLORER')}
        </span>
        <div className="sidebar-header-actions">
          <button className="sidebar-header-btn" title={locale === 'zh-CN' ? '新建文件' : 'New File'} aria-label={locale === 'zh-CN' ? '新建文件' : 'New File'} onClick={() => { setCreating('file'); setNewName(''); }}>
          ...
          <button className="sidebar-header-btn" title={locale === 'zh-CN' ? '新建文件夹' : 'New Folder'} aria-label={locale === 'zh-CN' ? '新建文件夹' : 'New Folder'} onClick={() => { setCreating('folder'); setNewName(''); }}>
          ...
          <button className="sidebar-header-btn" title={locale === 'zh-CN' ? '刷新' : 'Refresh'} aria-label={locale === 'zh-CN' ? '刷新' : 'Refresh'} onClick={() => window.dispatchEvent(new CustomEvent('loom:refresh-tree'))}>
          ...
          <button className="sidebar-header-btn" title={locale === 'zh-CN' ? '打开文件夹' : 'Open Folder'} aria-label={locale === 'zh-CN' ? '打开文件夹' : 'Open Folder'} onClick={onOpenFolder}>
          ...
          {workspacePath && (
            <button className="sidebar-header-btn" title={locale === 'zh-CN' ? '关闭文件夹' : 'Close Folder'} aria-label={locale === 'zh-CN' ? '关闭文件夹' : 'Close Folder'} onClick={onCloseFolder}>
        ...
        {workspacePath ? (
            ...
            {creating && (
              <div style={{ padding: '6px 10px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 6, alignItems: 'center' }}>
                <input
                  ...
                  placeholder={creating === 'file' ? (locale === 'zh-CN' ? '文件名.后缀' : 'filename.ext') : (locale === 'zh-CN' ? '文件夹名' : 'folder name')}
                  value={newName}
                ...
                <button className="settings-btn-sm" onClick={() => { setCreating(null); setNewName(''); }} aria-label={locale === 'zh-CN' ? '取消' : 'Cancel'}>×</button>
              </div>
            )}
          ...
          <div style={{ padding: '24px 16px', textAlign: 'center' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '12px', marginBottom: '12px' }}>
              {locale === 'zh-CN' ? '尚未打开文件夹' : 'No folder opened'}
            </p>
            <button className="settings-btn-sm primary" onClick={onOpenFolder}>
              {locale === 'zh-CN' ? '打开文件夹' : 'Open Folder'}
            </button>
          </div>
        )}
      </div>
