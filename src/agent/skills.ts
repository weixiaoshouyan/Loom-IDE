...
const BUILTIN_SKILLS: Skill[] = [
  ...
  // ========== Newly added builtin skills (see PR/workflow) ==========

  // --- Productivity extended ---
  {
    id: 'git-smart-commit',
    name: 'Git Smart Commit',
    description: 'Craft a Conventional Commit message from staged+unstaged changes (read-only, does NOT stage or commit)',
    category: 'productivity',
    prompt: [
      'Craft a Conventional Commit message for the current staged+unstaged changes. Read `git status` and `git diff` first.',
      'Output only the commit message in this form:',
      '',
      '<type>(<scope>): <short summary>',
      '',
      '<body explaining what changed and why in 1-3 lines>',
      '',
      'Use one of: feat, fix, docs, style, refactor, perf, test, chore, ci, build. Do NOT stage or commit; just return the message.',
      'Workspace: {workspace}',
    ].join('\n'),
    icon: '📦',
    builtin: true,
  },
  {
    id: 'run-tests',
    name: 'Run Tests',
    description: 'Detect and run the project test runner, then report results and propose fixes',
    category: 'productivity',
    prompt: [
      "Run the project's test suite safely. First detect the test runner in use (scripts in package.json / Cargo.toml / pytest / go test / etc.),",
      ' then invoke a non-destructive test command appropriate for that runner.',
      ' Capture stdout+stderr and report pass/fail counts.',
      ' If tests fail, list the failing test names and a one-line likely cause for each, and propose the smallest next edit to fix them.',
      '',
  ...
  },
  {
    id: 'format-file',
    name: 'Format File',
    description: 'Auto-format the current file with the project preferred formatter',
    category: 'productivity',
    prompt: [
      "Format the given file or workspace using the project's preferred formatter (prettier, dprint, rustfmt, gofmt, black, etc.).",
      ' If no formatter config is found, recommend installing one and explain why formatting matters.',
      ' Always run in a non-destructive way (no --fix on linters that mutate unrelated code).',
      '',
      'File: {file}',
      'Workspace: {workspace}',
    ].join('\n'),
  ...
  // --- Analysis extended ---
  {
    id: 'summarize-code',
    name: 'Summarize Code',
    description: 'Give a developer a quick mental model of code without reading the whole file',
    category: 'analysis',
    prompt: [
      'Summarize the provided code for a developer who needs a quick mental model but does NOT want to read the whole file.',
      ' Cover: purpose, the data flow, error handling, side effects, public API, and one concrete improvement opportunity.',
      '',
      'Code:',
      '{code}',
    ].join('\n'),
  ...
  // --- Code extended ---
  {
    ...
    description: 'Fix IDE/linter errors and warnings with the smallest safe edit',
    category: 'code',
    prompt: [
      'Read the diagnostics for the file (errors + warnings from the IDE or a linter).',
      ' For each one: explain the root cause in one sentence and produce the smallest safe edit.',
      ' Skip stylistic nits unless they break the build. Prefer exact_string_replace over full-file rewrites.',
      '',
      'File: {file}',
      'Diagnostics:',
      '{diagnostics}',
  ...
  // --- System extended ---
  {
    id: 'find-skill',
    name: 'Find Skill',
    description: 'Recommend the top 1-3 available skills for the given goal',
    category: 'system',
    prompt: [
      'You are a skill recommender. The user describes a goal. From the currently available skills, pick the top 1-3 that best match,',
      ' and for each explain in one sentence why it fits. If none fit well, say so plainly.',
...
export class SkillManager {
  ...
  addSkill(skill: Omit<Skill, 'builtin'> & { builtin?: boolean }) {
    if (!skill.id || !skill.name || !skill.prompt) {
      console.error('SkillManager: addSkill requires id, name, and prompt');
      return false;
    }
  ...
  resolvePrompt(skillId: string, variables: Record<string, string>): string | null {
    ...
    for (const [key, value] of Object.entries(variables)) {
      // 全局替换：同一变量在模板中可能出现多次（split/join 避开正则转义问题）
      prompt = prompt.split(`{${key}}`).join(value || `{${key}}`);
    }
    // Handle {code} specially - it's the most common
    const codeValue = variables['code'] || variables['selectedText'];
    if (codeValue) {
      prompt = prompt.split('{code}').join(codeValue);
    }
    return prompt;
  }
