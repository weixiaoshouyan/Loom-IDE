...
- **验证闭环**：改完自动跑类型检查/测试，失败继续修，通过才收工
- **长时间任务不卡死**：工具执行硬超时、流式读超时、命令超时不重试、可随时停止/继续
- **行内补全**：真 debounce 的 AI 补全，打字不烧额度
- **@codebase 语义检索**：Tree-sitter 代码索引，@ 引用文件/符号
...
# 端到端测试（真实 Electron：工作流 / 打包版 / Agent 面板，11 用例）
npm run e2e
...
| 测试 | Vitest（217）· Playwright e2e（11）|
